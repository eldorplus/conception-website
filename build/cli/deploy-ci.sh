#! /bin/sh
mkdir temp_web
git config --global user.name "Patrick LUZOLO"
...
if [ "$TRAVIS_TAG" ]; then
  ...
  npm run dist
  cd temp_web
  git clone https://$ROT_TOKEN@github.com/eldorplus/conception.git && cd conception
  ...
  # build theme-default
  cd temp_web
  git clone https://$ROT_TOKEN@github.com/eldorplus/theme-default.git && cd theme-default
  rm -rf *
  ...
  cd temp_web
  git clone -b gh-pages https://$ROT_TOKEN@github.com/eldorplus/element.git && cd element
  # build sub folder
  echo $TRAVIS_TAG
